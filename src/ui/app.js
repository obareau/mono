...
export function mountApp(root) {
    ...
    fileInput.addEventListener("change", async () => {
        ...
        if (f)
            store.setSource(await loadImageFile(f));
    });
    ...
    function renderSidebar() {
        ...
        store.stack.forEach((item, idx) => {
            ...
            tools.append(iconBtn("↑", "Move up", () => store.move(item.uid, -1)), iconBtn("↓", "Move down", () => store.move(item.uid, 1)), iconBtn(item.enabled ? "●" : "○", "Toggle", () => store.toggleFilter(item.uid)), iconBtn("✕", "Remove", () => store.removeFilter(item.uid)));
            bar.appendChild(tools);
            card.appendChild(bar);
            const body = el("div", "fcard-body");
            for (const def of f.params)
                body.appendChild(buildControl(item, def));
