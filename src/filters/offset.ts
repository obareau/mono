...
export const offset: Filter = {
  ...
  apply(gray, w, h, p) {
    ...
    if (mode === "slices") return slices(gray, w, h, p);
    return echo(gray, w, h, p);
  },
};

function combine(a: number, b: number, blend: string): number {
  switch (blend) {
    case "screen":
      return 1 - (1 - a) * (1 - b);
    case "difference":
...
function echo(gray: Float32Array, w: number, h: number, p: Record<string, number | string | boolean>): Float32Array {
  const dx = p.dx as number;
  const dy = p.dy as number;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const sy = y - dy;
      const ghost =
        sx >= 0 && sx < w && sy >= 0 && sy < h ? gray[sy * w + sx] : 1; // out-of-bounds = white (no ink)
...
function slices(gray: Float32Array, w: number, h: number, p: Record<string, number | string | boolean>): Float32Array {
  const bands = p.bands as number;
  const chaos = p.chaos as number;
  const maxShift = p.dx as number;
  const rng = mulberry32(p.seed as number);
  const out = new Float32Array(gray.length);
  const bandH = Math.max(1, Math.floor(h / bands));
  for (let y = 0; y < h; y++) {
    const band = Math.floor(y / bandH);
    // deterministic shift per band
    const r = mulberry32((p.seed as number) * 7919 + band)();
